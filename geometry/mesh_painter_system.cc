...
class PainterReifier final : public ShapeReifier {
 ...
 private:
  ...
  void ImplementGeometry(const Box& box, void*) final {
    ...
    std::cerr << "Box has " << mesh->num_elements() << " tets\n";
    auto bvh = make_unique<BoundingVolumeHierarchy<VolumeMesh<double>>>(*mesh);
    mesh_ = make_unique<PainterMesh>(move(mesh), move(bvh));
  }
...
class CanvasReifier final : public ShapeReifier {
 ...
 private:
  ...
  void ImplementGeometry(const Mesh& mesh_spec, void*) final {
    ...
    // produce "errors". Errors are only produced when it seems a line that is
    // _suggestive_ of an OBJ file but then fails. So, it is highly unlikely
    // this exception will be thrown.
    if (!ret || !err.empty()) {
      throw std::runtime_error(fmt::format(
          "Error parsing Wavefront obj file ({}): ", mesh_spec.filename(), err));
    }
    ...
    if (attrib.texcoords.size() == 0) {
      throw std::runtime_error(
          fmt::format("OBJ cannot be used as a paint canvas; it has no texture "
                      "coordinates: {}", mesh_spec.filename()));
    }
...
void RasterizeTriangle(const Vector2d& a, const Vector2d& b, const Vector2d& c,
  ...
  /* Known problems
      ...
      2. A triangle that spans (for example) a domain of uvs from [1 - a, 1 + b]
       ...
       it overlaps a corner, we need to do more.
   */
  const int w = image->width();
  const int h = image->height();

  // Maps uv coordinates to image space coordinates.
  ...
  const int max_x = std::min(w, static_cast<int>(max_x_d + 0.999));
  const int max_y = std::min(h, static_cast<int>(max_y_d + 0.999));
...
MeshPainterSystem::MeshPainterSystem(GeometryId mesh_id, GeometryId painter_id,
  ...
  texture_output_port_ = &this->DeclareAbstractOutputPort(
      "texture", texture_image, &MeshPainterSystem::CalcTextureImage);
}
