...
/** A variant of a QueryObject which performs spatial queries -- contact, signed
 distance, etc. -- in addition to the queries made available by QueryObject.
 SceneGraph has an abstract-valued port that contains a %SpatialQueryObject
 (i.e., a %SpatialQueryObject-valued output port).

 Other than the additional queries that this class provides above and beyond
 that of QueryObject, acquiring a reference to a %SpatialQueryObject, working
...
*/
template <typename T>
class SpatialQueryObject : public QueryObject<T> {
 public:
  DRAKE_DEFAULT_COPY_AND_MOVE_AND_ASSIGN(SpatialQueryObject)

  /** Constructs a default SpatialQueryObject (all pointers are null). */
  SpatialQueryObject() = default;
  ...
  /** Computes the penetrations across all pairs of geometries in the world
   ...
   taking responsibility to know whether or not SpatialQueryObject supports AutoDiff
