...
/** System for painting on a mesh. The painter system is associated with two
 ...
 @system{MeshPainterSystem,
   ...
   @output_port{texture} @output_port{scene_graph_texture}
 }
 */
class MeshPainterSystem final : public systems::LeafSystem<double> {
 public:
  ...
  /** Constructs a %MeshPainterSystem.
   ...
   @param name          The name of this image; see
                        SceneGraph::RegisterInputImage() for details about
                        restrictions on this name.
   @param scene_graph   The SceneGraph instances that contains the given ids.
                        This system will register itself with `scene_graph` as
                        a source and register an input image so that it can
                        connect.
   */
  ...
  /** The identifier for the image that this system produces. It will access the
   SceneGraph's port which is expecting this image.  */
  ImageId image_id() const { return image_id_; }
