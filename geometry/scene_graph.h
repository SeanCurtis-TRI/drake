...
class SceneGraph final : public systems::LeafSystem<T> {
 public:
  ...
  /** @name     Rendering "label" images

   %SceneGraph can be used to render "label" images (images where each pixel
   value is associated with the "class" of the geometry that drew to that
   pixel). %SceneGraph is responsible for managing the render labels that are
   associated  with the geometries it manages. When adding a perception role to
   geometry, it may be necessary to assign a render label to the geometry
   (depending on the render engines that have been added to the %SceneGraph
   instance).

   %SceneGraph knows nothing about the semantic meaning of the geometry classes.
   Geometry sources define the classes of geometry they know about by name and
   communicate these classes to %SceneGraph via the GetRenderLabel() method
   which returns a RenderLabel instance that contains the value that will be
   present in the rendered label image.
   ...
   There is no provision for returning allocated render labels back to the pool.
  ...
  /** systems::Context-modifying variant of
   @ref AssignRole(SourceId,GeometryId,ProximityProperties) "AssignRole()" for
   illustration properties. Rather than modifying %SceneGraph's model, it
