...
def render_time_vs_lights(data_dict):
    ...
    However, there's a separate line based on image size."""
    fig, axes = plt.subplots(2, len(data_dict), figsize=(10, 10))
    ...
    for engine_name, data in data_dict.items():
        ...
        _add_grouped_time_bars(
            ax=ax,
            data=data,
            title=f"Render Time vs Light Count\nOne sphere in {name}",
    ...
    for engine_name, data in data_dict.items():
        ...
        _add_grouped_time_bars(
            ax=ax,
            data=data,
            title=f"Render Time vs Sphere Count\n2560x1920 image in {name}",
            group_member=LIGHT_COUNT,
            x_axis=SPHERE_COUNT,
    ...
    fig.suptitle("Render Time vs Light Count")
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])  # Adjust layout to fit title
    plt.subplots_adjust(wspace=0.3)  # Adjust space between subplots
