...
// Although the method requires the box to be described in the plane's frame,
// to make sure that all the bits are working together properly, we will use
// a plane with an arbitrary normal direction in the P frame (and a non-zero
// displacement from Po).
GTEST_TEST(PlaneTest, BoxOverlap) {
  // We'll set up the problem in some frame C, where the plane normal is
  // [0, 0, 1] and the offset is zero. Once we've configured a box w.r.t. to
  // that frame, we'll re-express them all in a separate frame P.
  ...
  const std::vector<RotationMatrixd> R_PCs{
  ...
  const std::vector<double> clearances{
      R_PCs[0].row(2).cwiseAbs().cwiseProduct(half_width.transpose()).sum(),
      R_PCs[1].row(2).cwiseAbs().cwiseProduct(half_width.transpose()).sum(),
  };
  ...
  for (int i = 0; i < ssize(R_PCs); ++i) {
    boxes.push_back(
        Box{.center = Vector3d(0, 0, clearances[i] + 1e-10),
            .orientation = R_PCs[i],
