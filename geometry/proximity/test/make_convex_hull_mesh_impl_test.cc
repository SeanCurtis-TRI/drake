...
PolyMesh MakeCube(const Vector3d& scale) {
...
GTEST_TEST(MakeConvexHullMeshTest, GltfMesh) {
  ...
  const PolyMesh bin_cube = MakeCube(Vector3d::Ones());
...
GTEST_TEST(MakeConvexHullMeshTest, NonZeroMargin) {
  ...
  const Vector3d scale(2, 2, 2);
  const PolyMesh expected = MakeCube(scale + Vector3d::Constant(margin));
...
GTEST_TEST(MakeConvexHullMeshTest, MakeFromMeshSource) {
  ...
  const PolyMesh expected_box = MakeCube(kScale + Vector3d::Constant(kMargin));
