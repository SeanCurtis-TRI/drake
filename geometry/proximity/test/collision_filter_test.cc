...
class CollisionFilterTest : public ::testing::Test {
 protected:
  ...
  /* Creates a CollisionFilter with all the geometries as `filter` but with all
   pairs being unfiltered. */
  static CollisionFilter ClearCopy(const CollisionFilter& filter) {
    auto clear_state = CollisionFilter::InitializeTransientState(
        filter.filter_state_, CollisionFilter::kUnfiltered);
...
/* Exercise transient declaration API. This entails five different methods that
 ...
 function, we want to test certain properties (as listed below):
  A ApplyTransient()
    1 Can't undo permanent filters.
    2 Can add filters.
    3 Can remove filters.
  B has_active_history()
  ...
  E Flatten()
    1 Flattenging a flat history makes no difference.
    2 Any previous ids given are no longer valid.
    3 Reports no active history.
    4 Flatten preserves resultant state.
...
TEST_F(CollisionFilterTest, TransientDeclarations) {
  ...
  EXPECT_FALSE(filter.has_active_history());
  ...
  auto is_expected =
    ...
    for (const auto& pair : expected) {
      temp_filter.Apply(
          ...
          this->get_extract_ids_functor(), !permanent);
    }
    if (temp_filter != f) return ::testing::AssertionFailure();
    return ::testing::AssertionSuccess();
  };

  std::set<Pair> expected_filters;
  const CollisionFilter::ExtractIds& extract = this->get_extract_ids_functor();

  /* Set up the persistent base with pairs
    P(D, E), (B, C), (B, D), (B, E)
    * P(i, j) denotes a *permanent* pair between i and j. */
  filter.Apply(Decl().ExcludeWithin(GeometrySet{D, E}), extract, permanent);
  filter.Apply(Decl().ExcludeBetween(GeometrySet{B}, GeometrySet{C, D, E}),
               extract, !permanent);
  expected_filters.emplace(std::make_pair(D, E));
  expected_filters.emplace(std::make_pair(B, C));
  expected_filters.emplace(std::make_pair(B, D));
  expected_filters.emplace(std::make_pair(B, E));
  ASSERT_TRUE(is_expected(filter, expected_filters));
  ...
  const FilterId id0 =
      filter.ApplyTransient(CollisionFilterDeclaration().ExcludeBetween(
                            ...
                            this->get_extract_ids_functor());
  EXPECT_TRUE(is_expected(filter, expected_filters));
  /* Prop. B.2.: history -> true */
  EXPECT_TRUE(filter.has_active_history());
  ...
  /* Prop. A.2 and A.3: Add new filter (C, D), remove old, non-permanent
  ...
  expected_filters.emplace(std::make_pair(C, D));
  expected_filters.erase(expected_filters.find(std::make_pair(B, D)));
  ASSERT_TRUE(is_expected(filter, expected_filters));
  ...
  const CollisionFilter id1_state(filter);
  Decl add_2_clear_2;
  ...
  expected_filters.emplace(std::make_pair(A, B));
  expected_filters.emplace(std::make_pair(A, C));
  expected_filters.erase(expected_filters.find(std::make_pair(B, C)));
  expected_filters.erase(expected_filters.find(std::make_pair(B, E)));
  ASSERT_TRUE(is_expected(filter, expected_filters));
  ...
  ASSERT_TRUE(filter == id1_state);

  /* Put the declaration back on:
    P(D, E), (A, B), (A, C), (C, D) */
  ...
  expected_filters.erase(expected_filters.find(std::make_pair(A, C)));
  expected_filters.emplace(std::make_pair(B, C));
  ASSERT_TRUE(is_expected(filter, expected_filters));

  /* Prop. A.1: Attempting to remove permanent filter makes no difference:
    P(D, E), (A, B), (B, C), (C, D) */
  const FilterId id5 =
  ...
  expected_filters.erase(expected_filters.find(std::make_pair(A, B)));
  ASSERT_TRUE(is_expected(filter, expected_filters));

  /* Prop E: flattening history. Before flattening, we'll confirm valid filter
   ids and active history. */
  ASSERT_TRUE(filter.has_active_history());
