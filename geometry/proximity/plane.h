 ...
 public:
  ...
  /** Constructs a %Plane in frame F which is normal to `normal` and passes
   ...
   @param already_normalized
       ...
       been normalized by the caller. It should still have unit length. This
       function reserves the right to validate this property in debug build up
       to an arbitrary tolerance. When in doubt, allow the plane to normalize
       the normal vector.
   @pre If `already_normalized` is `false`, `normal` must have
        `magnitude ≥ 1e-10`.
   */
  ...
  promoted_numerical_t<U, T> CalcHeight(const Vector3<U>& Q) const {
    return nhat_F_.dot(Q) - displacement_;
  }
