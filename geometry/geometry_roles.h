...
/** @defgroup geometry_roles Geometry Roles
 ...
 lettering, etc. Furthermore, the materials the arm is made of has meaningful
 material properties (e.g., Young's modulus, Poisson ratio, etc.) All of these
 ...
 Drake has partitioned its geometric operations into three classes and has
 a unique role for each partition:
 ...
 <!-- TODO(SeanCurtis-TRI): Can we come up with a better name than "proximity"
   ...
   - __Proximity role__: these are the operations that are related to
     evaluations of the signed distance field between two geometries. When the
     ...
     properties don't affect the *geometric* operation, but are used in
 ...
 In both of those cases, the properties that are defined should be appropriate
 to the calculations performed (i.e., which contact model or which rendering
 method). The geometry properties support multiple parallel sets of properties
 *per role* (see GeometryProperties for how this mechanism works). To make a
 geometry generally compatible with anything in drake, it could provide
 properties for all known consumers of geometry properties (e.g., contact
 models, renderers, etc.)
