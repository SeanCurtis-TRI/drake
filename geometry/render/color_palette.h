...
namespace std {
template <typename T>
struct hash<drake::geometry::render::Color<T>>
...
class ColorPalette {
 public:
  ...
  ///
  /// @param terrain_label The label to express pixels which correspond to flat
  /// terrain. This will be used in the label image.
  ///
  /// @param empty_label The label to express pixels that have no body. This
  /// will be used in the label image.
  ///
  /// @throws std::logic_error When @p num_colors exceeds the maximum limit,
  /// which is 1535.
  ColorPalette(int num_colors, RenderLabel terrain_label,
    ...
    for (size_t i = 0; i < colors_.size(); ++i) {
      color_id_map_[colors_[i]] = RenderLabel(i);
    }
    color_id_map_[kTerrainColor] = terrain_label;
    color_id_map_[kSkyColor] = empty_label;
  }
  ...
  const ColorI& get_color(int index) const {
    if (index == RenderLabel::terrain_label()) {
  ...
  RenderLabel LookUpId(const ColorI& color) const {
 ...
 private:
  ...
  std::unordered_map<ColorI, RenderLabel> color_id_map_;
};
