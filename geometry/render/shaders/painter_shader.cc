...
optional<PainterShader> PainterShader::Make(
  ...
  const string& dynamic_mask =
      properties.GetPropertyOrDefault(kGroup, "dynamic_mask", "");
  const string& static_mask =
      properties.GetPropertyOrDefault(kGroup, "static_mask", "");
  if (dynamic_mask.empty() && static_mask.empty()) {
    throw std::runtime_error(
        "Error in declaration of painter shader; missing mask "
  ...
  DRAKE_ASSERT(dynamic_mask.empty() != static_mask.empty());
  const bool is_dynamic = !dynamic_mask.empty();
  const string& mask_name = is_dynamic ? dynamic_mask : static_mask;

  if (!is_dynamic && !std::ifstream(mask_name)) {
    throw std::runtime_error(
        fmt::format("Error in declaration of painter shader; static mask "
                    "file cannot be found: '{}'", mask_name));
  }
