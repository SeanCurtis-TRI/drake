...
// For RenderEngineGl this has a value of 1.001 to account for differences
// between mac and linux. Since this only runs on linux, we'll clamp the
// tolerance to 1.0.
const double kColorPixelTolerance = 1.01;
...
class RenderEngineGlTest : public ::testing::Test {
 ...
 protected:
  ...
  void Init(const RigidTransformd& X_WR, bool add_terrain = false) {
    RenderEngineGlParams params{.default_clear_color = kBgColor};
...
TEST_F(RenderEngineGlTest, ControlBackgroundColor) {
  ...
  for (const auto& bg : backgrounds) {
    RenderEngineGlParams params{.default_clear_color = bg};
  ...
  for (const bool use_texture : {false, true}) {
    ...
    for (const double texture_scale : {1.0, 1.5, 0.5}) {
      ...
      if (texture_scaled) {
        ...
        //  within 1/255 on each channel). Howver, as I increase the scale
