...
// Friend class to SpatialQueryObjectTest -- left in `drake::geometry` to match
// the friend declaration. The name gives it access to the private/protected
// access of SpatialQueryObject's *parent* class, QueryObject.
class QueryObjectTest : public ::testing::Test {
 protected:
  template <typename T>
  static ::testing::AssertionResult is_default(
      const SpatialQueryObject<T>& object) {
...
TEST_F(QueryObjectTest, DefaultQueryThrows) {
  SpatialQueryObject<double> default_object;
