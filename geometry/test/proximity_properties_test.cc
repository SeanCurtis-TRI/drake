...
GTEST_TEST(ProximityPropertiesTest, AddContactMaterial) {
  ...
  // Case: Correct configuration.
  {
    ...
    EXPECT_EQ(
        p.GetProperty<double>(internal::kMaterialGroup, internal::kElastic), E);
    EXPECT_EQ(p.GetProperty<double>(internal::kMaterialGroup,
                                    internal::kHcDissipation),
              d);
    const CoulombFrictiond& mu_stored = p.GetProperty<CoulombFrictiond>(
        internal::kMaterialGroup, internal::kFriction);
  ...
  {
    ProximityProperties p;
    p.AddProperty(internal::kMaterialGroup, internal::kElastic, E);
  ...
  {
    ProximityProperties p;
    p.AddProperty(internal::kMaterialGroup, internal::kHcDissipation, d);
    DRAKE_EXPECT_THROWS_MESSAGE(
        AddContactMaterial(E, d, mu, &p), std::logic_error,
        ".+ Trying to add property .+ to .+ property .+ already exists");
  }
  ...
  {
    ProximityProperties p;
    p.AddProperty(internal::kMaterialGroup, internal::kFriction, mu);
    DRAKE_EXPECT_THROWS_MESSAGE(
        AddContactMaterial(E, d, mu, &p), std::logic_error,
        ".+ Trying to add property .+ to .+ property .+ already exists");
  }
