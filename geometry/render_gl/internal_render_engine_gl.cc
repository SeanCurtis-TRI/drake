...
void RenderEngineGl::DoRenderLabelImage(const ColorRenderCamera& camera,
  ...
  // to the default RGBA window buffer. For show_window only, convert the
  // already-read labels into their legacy RGB encoding and display that.
  if (camera.show_window()) {
    ImageRgba8U display_image(label_image_out->width(),
                              label_image_out->height());
    for (int y = 0; y < label_image_out->height(); ++y) {
      for (int x = 0; x < label_image_out->width(); ++x) {
        const uint16_t label_bits =
            static_cast<uint16_t>(*label_image_out->at(x, y));
        uint8_t* pixel = display_image.at(x, y);
        pixel[0] = static_cast<uint8_t>(label_bits & 0xFF);
        pixel[1] = static_cast<uint8_t>((label_bits >> 8) & 0xFF);
        pixel[2] = 0;
        pixel[3] = 255;
      }
    }
