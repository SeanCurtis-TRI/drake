...
// Mock classes to get access to the internal components of ProximityEngine
class ProximityEngineTester {
 public:
  ProximityEngineTester() = delete;

  template <typename T>
  static void UpdateWorldPoses(const std::vector<Isometry3<T>>& poses,
                               internal::ProximityEngine<T>* engine) {
    engine->UpdateWorldPoses(poses);
  }
};
}  // namespace internal

// Mock classes to get access to the internal components of GeometryState
template <typename T>
class GeometryStateTester {
 public:
  ...
  internal::ProximityEngine<T>* get_engine() {
...
using internal::ProximityEngineTester;
...
class GeometryQueryTest : public ::testing::Test {
 protected:
  void SetUp() override {
    // Note: We assume the
    context_ = std::make_unique<GeometryContext<double>>();
