...
class InternalGeometry {
 public:
  ...
  /** Returns the pose of this geometry in the frame to which it is ultimately
   rigidly attached. This is in contrast to X_PG().  */
  void set_X_FG(const Isometry3<double>& X_FG) { X_FG_ = X_FG; }
  ...
  /** Returns a mutable list of child geometry identifiers.  See
   child_geometry_ids()` for details.  */
  ...
  bool is_dynamic() const {
    return frame_id_ != InternalFrame::get_world_frame_id();
  }
