...
EnvironmentTexture ReadEquirectangularFile(std::string const& fileName) {
  ...
  // Note: This is the string that vtkHDRReader::GetFileExtensions() returns.
  // However, it isn't a static function, so we simply reproduce it here.
  const bool is_hdr =
      std::string(".hdr .pic").find(extension, 0) != std::string::npos;
