  ...
  if (i1.width() != i2.width() || i1.height() != i2.height()) {
    ...
    for (int r = 0; r < i1.height(); ++r) {
      for (int c = 0; c < i1.width(); ++c) {
        const RgbaColor c1(i1.at(c, r));
        const RgbaColor c2(i2.at(c, r));
...
TEST_F(RenderEngineVtkTest, BadSharedTexture) {
  ...
  const ColorRenderCamera color_camera(core, kShowWindow);
  ...
  if (const char* dir = std::getenv("TEST_UNDECLARED_OUTPUTS_DIR")) {
    auto save_image = [dir](const ImageRgba8U& image, const std::string& name) {
      systems::sensors::SaveToPng(image,
                                  std::filesystem::path(dir) / (name + ".png"));
    };
    save_image(original, "image_A1");
    save_image(original2, "image_A2");
    save_image(original3, "image_A3");
    save_image(clone, "image_B1");
    save_image(clone2, "image_B2");
  }
