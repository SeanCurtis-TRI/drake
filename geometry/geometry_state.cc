...
GeometryId GeometryState<T>::RegisterGeometry(
  ...
  if (child_count > 1) {
    // Filter collisions between geometries affixed to the same frame. This
    // directly exercises the collision cliques. We only add a clique once there
    // are multiple child geometries.
    ...
    if (child_count > 2) {
      ...
      // We've reached *multiple* child geometries -- assign to clique.
...
void GeometryState<T>::DisallowSelfCollisions(
  ...
  CollectIndices(group, &dynamic, &anchored);

  geometry_engine_->DisallowSelfCollisions(dynamic, anchored);
}

template <typename T>
void GeometryState<T>::DisallowCrossCollisions(const CollisionGroup& group1,
                                               const CollisionGroup& group2) {
  std::unordered_set<GeometryIndex> dynamic1;
  std::unordered_set<AnchoredGeometryIndex> anchored1;
  CollectIndices(group1, &dynamic1, &anchored1);
  std::unordered_set<GeometryIndex> dynamic2;
  std::unordered_set<AnchoredGeometryIndex> anchored2;
  CollectIndices(group2, &dynamic2, &anchored2);

  geometry_engine_->DisallowCrossCollisions(dynamic1, anchored1, dynamic2,
                                            anchored2);
}
...
template <typename T>
void GeometryState<T>::CollectIndices(const CollisionGroup& group,
                std::unordered_set<GeometryIndex>* dynamic,
                std::unordered_set<AnchoredGeometryIndex>* anchored) {
  for (auto frame_id : group.frames()) {
  ...
  for (auto geometry_id : group.geometries()) {
