...
void GeometryState<T>::FinalizePoseUpdate() {
  ...
  std::vector<RigidTransform<T>> X_WG_RT;
  std::transform(X_WG_.begin(), X_WG_.end(), std::back_inserter(X_WG_RT),
                 [](const Isometry3<T>& pose_in) {
                   return RigidTransform<T>(pose_in);
                 });
  for (auto& pair : render_engines_) {
    pair.second->UpdatePoses(X_WG_RT);
  }
}
