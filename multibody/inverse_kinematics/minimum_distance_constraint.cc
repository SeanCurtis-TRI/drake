...
VectorX<S> Distances(const MultibodyPlant<T>& plant,
  ...
  if (!query_port.HasValue(*context)) {
    throw std::invalid_argument(
        ...
        "geometry::SpatialQueryObject. Either the plant "
        "geometry_query_input_port() is not properly connected to the "
        "SceneGraph's SpatialQueryObject output port, or the plant_context_ is "
        "incorrect. Please refer to AddMultibodyPlantSceneGraph on connecting "
        "MultibodyPlant to SceneGraph.");
  }
  const auto& query_object =
      query_port.template Eval<geometry::SpatialQueryObject<T>>(*context);
...
void MinimumDistanceConstraint::Initialize(
  ...
  const int num_collision_candidates =
      query_port.template Eval<geometry::SpatialQueryObject<T>>(*plant_context)
          .inspector()
