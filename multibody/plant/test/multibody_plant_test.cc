...
using geometry::SpatialQueryObject;
 ...
 public:
  ...
  ComputePointPairPenetration() const {
    ...
    const auto& query_object = plant_->get_geometry_query_input_port().
        Eval<geometry::SpatialQueryObject<double>>(*plant_context_);
...
GTEST_TEST(MultibodyPlantTest, VisualGeometryRegistration) {
  ...
  unique_ptr<AbstractValue> state_value =
      scene_graph.get_spatial_query_output_port().Allocate();
  DRAKE_EXPECT_NO_THROW(state_value->get_value<SpatialQueryObject<double>>());
  const SpatialQueryObject<double>& query_object =
      state_value->get_value<SpatialQueryObject<double>>();
  scene_graph.get_spatial_query_output_port().Calc(*context, state_value.get());
