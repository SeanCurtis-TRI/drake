...
// Struct to store the specifications for a distance constraint.
// Distance constraints are modeled as a holonomic constraint. This struct
// allows to specify "soft" distance constraints, which impose the condition:
//   (d(q)-d₀) + c/k⋅ḋ(q) + 1/k⋅f = 0
// where d₀ is a fixed length, k a stiffness parameter in N/m and c a damping
// parameter in N⋅s/m. This constraint reduces to d(q) = d₀ in the limit to
// infinite stiffness and it behaves as a linear spring damper for finite values
