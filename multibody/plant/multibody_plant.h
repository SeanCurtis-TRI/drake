...
class MultibodyPlant : public internal::MultibodyTreeSystem<T> {
 public:
  ...
  /// it is called a "free body" (sometimes called a "floating body").
  /// Frequently, the root body of trees in the forest may be "free bodies". But
  /// bodies in the interior of a tree can likewise be "free"; it simply
  /// requires six degrees of freedom w.r.t. its parent body.
  ...
  /// explicitly accessing the corresponding floating joint. But the fact that
  /// free bodies have joints is reflected in the APIs. The pose of a floating
  /// body (as it is represented in %MultibodyPlant's state) is *always* the
  /// pose of the free body relative to its parent frame. For bodies that
  /// acquire a floating joint at the time of Finalize(), that parent frame is
  /// the world frame and the pose of _those_ free bodies are X_WB. However,
  /// if a floating joint is explicitly added from body A to body B, the pose
  /// is X_AB. If you want to express the pose of body B in the world, you
  /// still call these APIs with the local transform as: X_AB = X_WA⁻¹·X_WB.
  /// @{
  ...
  /// @note As documented above, this is the body's configuration in
  /// %MultibodyPlant state, which is the configuration of the floating joint
  /// between  `body` and its parent frame. If the parent frame is world, this
  /// is X_WB. Otherwise, it is X_PB for arbitrary P. To acquire X_WB,
