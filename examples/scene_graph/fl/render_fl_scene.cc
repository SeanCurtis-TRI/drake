...
DEFINE_int32(width, 2560, "Image width in pixels");
DEFINE_int32(height, 2048, "Image height in pixels");
...
DEFINE_bool(gl, true, "If true, gl renderer, if false, vtk renderer");
DEFINE_int32(render_samples, 10, "Number of renderings to make");
...
int do_main() {
  ...
  if (FLAGS_gl) {
    ...
    scene_graph.AddRenderer(renderer_name, MakeRenderEngineGl(params));
  } else {
    std::cout << "Adding VTK renderer.\n";
    geometry::render::RenderEngineVtkParams params;
    params.default_clear_color = Vector3d{0, 0, 0};
    scene_graph.AddRenderer(renderer_name, MakeRenderEngineVtk(params));
  }
  multibody::Parser parser(&plant, &scene_graph);
  {
    ...
    /* TODO(SeanCurtis-TRI) The kitchen scenario seems to include kitchen walls;
     I'm omitting them because, for the default camera location, they occlude
     the view of the kitchen. However, the reference image seems to include
     *some* backdrop outside the window. Find out what it is. It doesn't appear
     to be:
    ...
    std::cout << "  " << elapsed.count() << " ms to load assets\n";
  }
  plant.Finalize();
