  ...
  explicit ContactResultMaker(bool use_strict_hydro = true)
    ...
    geometry_query_input_port_ =
        this->DeclareAbstractInputPort("query_object",
                                       Value<SpatialQueryObject<double>>())
            .get_index();
 ...
 private:
  void CalcContactResults(const Context<double>& context,
    ...
    const auto& query_object =
        get_geometry_query_port().Eval<SpatialQueryObject<double>>(context);
...
int do_main() {
  ...
  builder.Connect(scene_graph.get_spatial_query_output_port(),
