...
TEST_F(RgbdSensorTest, PortNames) {
  RgbdSensor sensor("sensor", Vector3d(0, 0, 0), RollPitchYawd(0, 0, 0),
                    properties_);
...
TEST_F(RgbdSensorTest, ConstructAnchoredCamera) {
  ...
  auto make_sensor = [this, &p_WB_W, &rpy_WB_W, &name](SceneGraph<double>*) {
    return make_unique<RgbdSensor>(name, p_WB_W, rpy_WB_W, properties_);
  };
...
GTEST_TEST(RgbdSensorDiscrete, Construction) {
  ...
  RgbdSensorDiscrete sensor(
      make_unique<RgbdSensor>("sensor", Vector3d(0, 0, 0),
                              RollPitchYawd(0, 0, 0), properties),
...
GTEST_TEST(RgbdSensorDiscrete, ImageHold) {
  DepthCameraProperties properties(640, 480, M_PI / 4, "render", 0.1, 10);
  auto sensor = make_unique<RgbdSensor>("sensor", Vector3d(0, 0, 0),
                                        RollPitchYawd(0, 0, 0), properties);
