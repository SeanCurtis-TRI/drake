...
class RgbdSensor final : public LeafSystem<double> {
 public:
  ...
  /** Constructs an %RgbdSensor that is fixed to the world frame with the given
   pose and camera `properties`.
  ...
  RgbdSensor(std::string name, const Eigen::Vector3d& p_WB_W,
             const math::RollPitchYawd& rpy_WB_W,
