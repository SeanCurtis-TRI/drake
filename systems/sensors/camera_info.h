...
- frame, coordinate system, pose: denotes 2D/3D spaces, as in
...
The camera frame is described as follows:

- origin located at camera aperature
- +Z-axis aligned with the optical axis along the viewing direction,
  orthogonal to the image plane.
- +X-axis aligned with the right direction of the image plane
- +Y-axis aligned with the downwards direction of the image plane.

This can be summarized as `X-right`, `Y-down`, and `Z-forward`.

The image plane can be described the following coordinate systems:
