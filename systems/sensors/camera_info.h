...
/**
 ...
 appears. In essence, the parameters of the the camera model define a mapping
 from points in 3D space to a point on the image (2D space).
 ...
 can be captured by `A`, the camera or intrinsic matrix which essentially maps
 points in the camera frame C to the image plane:
 ...
 In other words, for point Q in the world frame, it's texture coordinates
 `(uₚ, vₚ)` are calculated as:
 ...
 - __aperture__: the opening in a camera through which light passes. The origin
   of the camera frame is located at the aperture's center.
 ...
 - __focal length__: a camera property that determines the field of view angle
   ...
   - in the pinhole model, `(f_x, f_y)` is described as "focal length", but the
     ...
     analogous affects on field of view and scale of objects in the image.
 ...
 - __image frame__: the 2D frame spanning the image plane. The image frame's
   x- and y-axes are parallel with `Cx` and `Cy`, respectively. Coordinates are
   expressed as the pair `(u, v)` and the camera's image lies on the plane
   spanned by the frame's basis, entirely in the first quadrant (i.e, `u ≥ 0`
   and `v ≥ 0`).
 - __image plane__: a plane in 3D which is perpendicular to a camera's viewing
   direction and positioned "in front" of the camera.
