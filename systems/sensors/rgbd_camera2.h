...
/** An RGB-D camera system that provides RGB, depth, and label images using
 the geometry in the geometry::SceneGraph.
...
class RgbdCamera2 final : public LeafSystem<double> {
 public:
  ...
  /** Constructs an %RgbdCamera2 whose frame `B` is the frame indicated by
  ...
  /** Returns the geometry::QueryObject-valued input port. */
 ...
 * a discrete sensor.
 */
class RgbdCameraDiscrete2 final : public systems::Diagram<double> {
 public:
  DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(RgbdCameraDiscrete2);
  ...
  RgbdCameraDiscrete2(std::unique_ptr<RgbdCamera2> camera,
  ...
  /// @see RgbdCamera2::query_object_input_port().
  const InputPort<double>& query_object_input_port() const {
