...
RgbdSensor::RgbdSensor(std::string name, const Vector3d& p_WB_W,
                       const RollPitchYawd& rpy_WB_W,
                       const DepthCameraProperties& properties,
                       bool show_window)
    : RgbdSensor(move(name), SceneGraph<double>::world_frame_id(),
                 RigidTransformd(rpy_WB_W, p_WB_W),
