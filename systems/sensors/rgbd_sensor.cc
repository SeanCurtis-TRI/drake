...
using geometry::RenderQueryObject;
...
RgbdSensor::RgbdSensor(FrameId parent_id,
  ...
  query_object_input_port_ = &this->DeclareAbstractInputPort(
      "geometry_query", Value<RenderQueryObject<double>>{});
...
void RgbdSensor::CalcColorImage(const Context<double>& context,
  ...
  const RenderQueryObject<double>& query_object = get_query_object(context);
...
const RenderQueryObject<double>& RgbdSensor::get_query_object(
    const Context<double>& context) const {
  return query_object_input_port().
      Eval<RenderQueryObject<double>>(context);
}
