...
PngWriter::PngWriter(const std::string& image_name, double start_time,
                     int padding)
...
void PngWriter::set_publish_period(double period) {
  LeafSystem<double>::DeclarePeriodicPublish(period);
}

const InputPort<double>& PngWriter::color_image_input_port() const {
  return this->get_input_port(image_port_index_);
}

void PngWriter::DoPublish(
