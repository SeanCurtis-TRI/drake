...
TEST_F(CompliantContactModelTest, ModelSingleCollision) {
  ...
  const double force = kContactStiffness * offset * 2;
...
class CompliantHeterogeneousModelTest : public CompliantContactModelTest {
 protected:
  ...
  // there is just a normal force with zero relative velocity. Stiffness is the
  // only parameter that matters.
  static const double kMaterialStiffness[2];

  static double contact_stiffness() {
    return kMaterialStiffness[0] * kMaterialStiffness[1] /
           (kMaterialStiffness[0] + kMaterialStiffness[1]);
  }
};

const double CompliantHeterogeneousModelTest::kMaterialStiffness[2]{10000,
                                                                    20000};
...
TEST_F(CompliantHeterogeneousModelTest, ModelSingleCollision) {
  ...
  double force_sign = -1;
  double s1 =
      kMaterialStiffness[1] / (kMaterialStiffness[0] + kMaterialStiffness[1]);
  if (b2 == body1_) {
    force_sign = 1;
    s1 = 1 - s1;
  }
  ...
  // NOTE: Each sphere is moved toward the other offset distance which makes
  // the penetration depth 2 * offset.
  const double force = contact_stiffness() * (offset * 2);
  expected_spatial_force << 0, 0, 0, force_sign * force, 0, 0;
  ASSERT_TRUE(
      CompareMatrices(resultant.get_spatial_force(), expected_spatial_force));
