...
void ExpectSuccessfulTransmogrification(
  ...
  auto test_map_equality = [](auto test_map, auto ref_map) {
    EXPECT_EQ(test_map.size(), ref_map.size());
    for (const auto& test_pair : ref_map) {
      EXPECT_EQ(ref_map.count(test_pair.first), 1);
      EXPECT_EQ(test_pair.second, ref_map[test_pair.first]);
    }
  };

  test_map_equality(ad_tester.get_source_name_map(),
                    d_tester.get_source_name_map());
  test_map_equality(ad_tester.get_source_frame_id_map(),
                    d_tester.get_source_frame_id_map());
  test_map_equality(ad_tester.get_source_root_frame_map(),
                    d_tester.get_source_root_frame_map());
  test_map_equality(ad_tester.get_source_anchored_geometry_map(),
                    d_tester.get_source_anchored_geometry_map());
  test_map_equality(ad_tester.get_geometries(), d_tester.get_geometries());
  test_map_equality(ad_tester.get_frames(), d_tester.get_frames());
  test_map_equality(ad_tester.get_anchored_geometries(),
                    d_tester.get_anchored_geometries());

  // 2. Test the vectors of ids
  auto test_vector_equality = [](auto& test, auto& reference) {
    EXPECT_EQ(test.size(), reference.size());
    for (size_t i = 0; i < test.size(); ++i) {
      ASSERT_EQ(test[i], reference[i]);
    }
  };

  test_vector_equality(ad_tester.get_geometry_index_id_map(),
                       d_tester.get_geometry_index_id_map());
  test_vector_equality(ad_tester.get_anchored_geometry_index_id_map(),
                       d_tester.get_anchored_geometry_index_id_map());
  test_vector_equality(ad_tester.get_pose_index_frame_id_map(),
